...
霜语支持将游戏聊天消息实时转发到 Telegram，并支持远程发送消息到游戏。

**功能特点：**
- 实时转发游戏消息到 Telegram
- 支持只转发筛选后的消息，或转发所有消息
- 远程命令：通过 Telegram 发送消息到游戏聊天
- 查询房间玩家列表
- 配置按账号独立保存
...
1. **创建 Telegram Bot**
   ...
   - 发送 `/newbot` 创建新机器人
   - 按提示设置名称，获得 Bot Token

2. **获取 Chat ID**
   - 私聊方式：向你的 Bot 发送任意消息，然后访问：
     ```
     https://api.telegram.org/bot<你的BotToken>/getUpdates
     ```
     在返回的 JSON 中找到 `chat.id`
   - 群组方式：将 Bot 加入群组，发送消息后同样访问上述链接获取群组 ID

3. **禁用隐私模式（重要）**
   - 找 @BotFather 发送 `/mybots`
   - 选择你的 Bot
   - Bot Settings → Group Privacy → 选择 **Disable**
   - 或者在群组中将 Bot 设为管理员
...
**💡 推荐使用群组：**
- 建议创建一个 Telegram 群组用于接收消息
- 将 Bot 添加到群组并设为管理员
- 管理员权限可让 Bot 删除用户发送的命令消息，保持聊天整洁
- Bot 会自动删除你发送的命令消息，只保留执行结果

**进入房间通知：**
- 进入房间时会发送带时间标签的通知
- 格式：`🚪 进入房间 #20260322_111307_房间名`
- 可通过标签快速搜索历史记录

**多账号支持：**
- 每个游戏账号独立保存 Telegram 配置
- 不同账号可以使用同一个 Bot，发送到不同的 Chat ID
- 适合多账号管理或多人共用 Bot 的场景
